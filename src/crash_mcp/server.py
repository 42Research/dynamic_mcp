...
class CrashMCPServer:
    ...
    def __init__(self):
        ...
        self.dynamic_connector = DynamicConnector(self.config, self)
        self._setup_tools()
            ...
            if scope["type"] == "http":
                ...
                if path == "/sse":
                    ...
                    try:
                        ...
                        # Handle request through Dynamic connector
                        result = await self.dynamic_connector.handle_dynamic_request(method, params)
                        ...
                        logger.error(f"Dynamic request endpoint error: {e}")
                        # Send error response
                        await send({
                            'type': 'http.response.start',
                            'status': 500,
    ...
    async def run_http(self, host: str = "0.0.0.0", port: int = 8080):
        ...
        # Start Dynamic connector if enabled
        await self.dynamic_connector.start()
        ...
        try:
            ...
            # Stop Dynamic connector
            await self.dynamic_connector.stop()
